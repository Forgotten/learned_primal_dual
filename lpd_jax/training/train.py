...
def create_train_state(
    ...
    if init_lr <= 0:
        raise ValueError(
            f"Learning rate must be strictly positive, got {init_lr}."
        )
    ...
    schedule = optax.cosine_decay_schedule(
        init_value=init_lr, decay_steps=decay_steps
    )
    tx = optax.chain(
...
def make_train_step() -> Callable:
    ...
    def train_step(
        ...
        (loss, x_pred), grads = jax.value_and_grad(loss_fn, has_aux=True)(
            state.params
        )
