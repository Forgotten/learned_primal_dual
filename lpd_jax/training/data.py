...
def generate_batch(
  ...
  """
  if batch_size < 1:
    raise ValueError(f"Batch size must be strictly positive, got {batch_size}.")
  ...
  noise = jax.random.normal(jax_rng, sino.shape) * noise_level * jnp.mean(jnp.abs(sino))

  sino_noisy = sino + noise

  # Expand dims to add channel.
  return jnp.expand_dims(sino_noisy, axis=-1), jnp.expand_dims(x_true_jax, axis=-1)
