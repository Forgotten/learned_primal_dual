...
def make_parallel_beam_geometry(
  ...
  """
  if len(img_shape) != 2:
    raise ValueError(f"Image shape must contain exactly 2 integers, got {img_shape}.")
  if len(img_extent) != 2:
    raise ValueError(f"Image extent must contain exactly 2 floats, got {img_extent}.")
