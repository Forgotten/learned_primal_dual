...
def _radon_single_angle(
  ...
  """
  if image.ndim != 2:
    raise ValueError(f"Image must be a 2D array, got {image.ndim} dimensions.")
...
@jax.jit
def radon_forward(image: jnp.ndarray, geom: ParallelBeamGeometry) -> jnp.ndarray:
  """
...
@jax.jit
def radon_adjoint(sinogram: jnp.ndarray, geom: ParallelBeamGeometry) -> jnp.ndarray:
  """
...
def make_batched_radon_forward(
  ...
  def _batched_fw(image_batch: jnp.ndarray) -> jnp.ndarray:
    if image_batch.ndim < 3:
      raise ValueError("Input to batched forward must have at least 3 dimensions.")
...
def make_batched_radon_adjoint(
  ...
  def _batched_adj(sino_batch: jnp.ndarray) -> jnp.ndarray:
    if sino_batch.ndim < 3:
      raise ValueError("Input to batched adjoint must have at least 3 dimensions.")
    # Remove channel.
